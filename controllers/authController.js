...
if (!['admin','client'].includes(role)) return res.status(400).json({ error: 'Role inválida' });


const sql = 'INSERT INTO users (name, cpf, role) VALUES (?,?,?)';
db.run(sql, [name, cpf, role], function(err) {
if (err) return res.status(400).json({ error: 'CPF já cadastrado ou erro' });
...
if (!name || !cpf) return res.status(400).json({ error: 'Dados incompletos' });
db.get('SELECT * FROM users WHERE cpf = ? AND name = ?', [cpf, name], (err, row) => {
