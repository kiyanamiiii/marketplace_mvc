...
async function load() {
  ...
  try {
    ...
    (Array.isArray(list) ? list : []).forEach(p => {
      ...
      meta.innerHTML = `R$ ${price.toFixed(2)} — <span style="display:block; margin-bottom:8px;">Estoque: ${stock}</span>`;
      ...
      editBtn.addEventListener('click', async () => {
        ...
        try {
          const r = await fetch(`${API}/${id}`, {
            ...
            body: JSON.stringify({ name: newName, description: '', price: newPrice, stock: newStock })
          });
          if (!r.ok) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('create').addEventListener('click', async () => {
    ...
    if (Number.isNaN(price) || Number.isNaN(stock)) { alert('Preço ou estoque inválido'); return; }
